...
library(readxl)
library(lubridate)
file <- here::here("data/raw/Manuscript one modeling dataset additional 13Sep2019.xlsx")
```
...
wol <- wol %>% mutate(across(.cols = everything(), as.numeric))
...
```{r}
releases <- read.csv(here::here("data/raw/release_dates.csv"), stringsAsFactors = FALSE)
...
```{r}
saveRDS(wol, here::here("data/clean/wol.rds"))
```
